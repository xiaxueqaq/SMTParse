...
void to_mathematica(const std::string& file_name, bool is_rename,std::ostream & o)
{
    auto t1=clock();
    ...
    //o<<"Eample:=";
    cnf.to_mathematica(o,file_name);
    //o<<":";
...
int main(int argc, char const *argv[])
{
    ...
    if (!file_name.empty())
    {
        ...
        if (!file_out_name.empty())
        {
            ...
            to_mathematica(file_name,is_rename,o);
            o.close();
        }
        else{
            to_mathematica(file_name,is_rename,std::cout);
        }
        // std::cout<<smt::ddd;
    }
    else{
                ...
                if (it.path().string().size()>=5 && it.path().string().substr(it.path().string().size()-5,it.path().string().size())==".smt2")
                {
                    ...
                    bool is_exist_unsimpleeq=false;
                    {
                        ...
                        try
                        {
                            ...
                            //fout<<"Eample:=";
                            cnf.to_mathematica(fout,it.path().string());
